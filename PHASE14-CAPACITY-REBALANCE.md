...
## Progress & status (2026-06-05)

- **14a ✅ shipped** (`9adf5a0`) — the `effectiveTarget` seam, byte-identical no-op (flag off).
- **14b ✅ shipped** (`da3e4b3`) — **the core structural win.** Cut `maxPerDay` (farm 50→36, mine 24→22, bakery 45→35, factory 24→22, diner 45→34, goods 24→21) so capacity now **binds**: brain-off steady-state utilization went from ~0.4 to **~0.85**. Paired with recentring the pricing neutral band to `[0.3, 0.92]` (`PRICE_UTIL_LOW/HIGH`) so prices still settle at base at the new tight capacity. 267 tests green (incl. the 365-day soak, conservation, no-bankruptcy, P9-9), **no re-baselining needed**, browser-healthy (8/8 active, fed, conserved, no errors). In the *live* config the tighter capacity already triggers investment (capital deepened 800→~1285 early) — though still transient without 14c.
- **14c ⛔ BLOCKED (reverted to 14b).** Target-scaling + a higher gate did NOT sustain the agentic loop — `investedDays` ~0 over 730 days, capital flat at baseline. **Root cause (traced through several layers, definitively):** the *agentic* supply chain collapses on a long run. Two pre-existing mechanisms, both **sharpened by the tighter 14b capacity buffers**: (1) **P10-7 producer cash-starvation → bankruptcy** — the B2B producers (esp. mine/factory) run out of cash and go bankrupt, laying off all staff; (2) **P10-3 labour concentration** — agentic residents switch jobs toward the higher-paying storefronts, draining producers of staff. Either way a producer hits **0 staff → 0 output → storefronts get no inputs → utilization reads 0** (masked), so the invest gate never fires on the firms that would invest. (The 14b browser saw only the *early transient* investment before this long-run collapse.) A side-investigation — the rules brain's `produce` lever flooding inventory — was tried as the suspected cause and **reverted**: it wasn't the blocker (utilization stayed masked) and it broke 2 tests.
- **The real prerequisite (next step):** make the agentic supply chain *survivable* before closing the loop — fix **producer financial viability (P10-7)** so B2B producers don't bankrupt, and/or temper the **labour concentration (P10-3)** so producers keep staff. This is **North-Star move #2 (firm health / business-entry) territory** — a substantial dedicated effort, not part of the capacity rebalance itself. Once producers survive agentic play, 14b's capacity-binding + the `effectiveTarget` seam (already in place, flag off) should let 14c (target-scaling + gate 0.80) close the loop. **14b's capacity-binding (util ~0.4 → ~0.85) is a real, shipped improvement that stands meanwhile.**
