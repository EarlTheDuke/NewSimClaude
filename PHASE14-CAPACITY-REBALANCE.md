...
- **14c ⛔ BLOCKED (reverted to 14b).** Turning on target-scaling + raising the gate did NOT sustain the loop in the **agentic** config: `investedDays` stayed ~0–3 over 730 days and capital spiked then decayed back to baseline (1821→701). **Root cause (newly found):** the rules brain's `produce` lever (`RuleBasedProvider`: "if inventory < 60, produce 80") tops up a storefront's `inventory` directly, so `make = min(capacity, target − inventory)` reads ~0 and **utilization is masked far below the real ~0.85**. The invest gate keys off utilization, so the *agentic* firms (the ones that invest) never see the capacity pressure the *brain-off* city has. The lock-step target-scaling can't hold an operating point the signal can't even read.
- **The fix (next step):** rework the rules brain's `produce` lever so it no longer bypasses the market's own demand-driven production (the market already refills storefronts via `make` in `MarketSystem.produce()`), letting utilization reflect real capacity pressure for agentic firms. Then re-attempt 14c (target-scaling + gate 0.80) and proceed to 14d/14e. This is a Phase-2-mechanic change with test impact (`decision.test` "produces when inventory runs low", competition, ceoBench) — a careful, contained piece, not a one-liner. **14b's capacity-binding is a real, shipped improvement that stands meanwhile.**
