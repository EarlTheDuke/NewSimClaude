...
- **13b — Engage quantity-only.** Raise `WEALTH_ELASTICITY` to ~1.0; residents above $500 buy 2+ units/visit. Demand ceiling lifts; utilization climbs. **Required (per validation):** restate the food-count invariants in `competition.test.ts:101-107` and `elasticity.test.ts:42` to "price-inelastic but **wealth-elastic**" (counts now measure units, not residents) and widen tolerances; re-baseline `macro.test.ts` (consumption/GDP now rise — expected); make the quantity test phase-aware (assert a known-id resident or a range, not exactly 2). **Wire the benchmark freeze for real**: thread a `wealthElasticity` option through `createCity → EconomySystem`, have `ceoBench` pass a frozen `BENCH_WEALTH_ELASTICITY`, and add a golden-scorecard test — so turning up the city-wide knob never silently drifts historical CEO scores. (`macro.test.ts:84-93` does NOT need re-baselining — brain-off keeps `investment=0`, so `gdp==consumption` stays true.)
- **13c — Tune + soak + realism axes + loop-closure.** Pick the final elasticity (likely ~0.5 diminishing-returns so the rich tail doesn't explode GDP). Graft the deferred realism axes: "fancier" (wealth term in `leisureReservation`) and optionally "more often" (`effHungry/effLonely` in `BrainSystem`). **Land the invest-before-distribute ordering fix here (or in its own slice) and update the `capital.test.ts:331-363` `investedDays===0` lock in the same change** — only then assert the invest lever fires and GDP climbs-then-plateaus (Solow) over a year.

**Invariants & risks.** Conservation (every unit a capped transfer) · determinism (pure, RNG-free; no new draws — the decisive win over the rejected stochastic-rounding design) · `WEALTH_BASELINE` must track cityGen's $500 start (pinned by a test) · inventory starvation / GDP over-shoot from the rich tail → bound by `WEALTH_DEMAND_CAP` + diminishing-returns elasticity, watched in the 13c soak · units 2..N are discretionary splurge (needs already satisfied) — keep the loop confined to EconomySystem.
