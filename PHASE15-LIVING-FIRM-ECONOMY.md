...
- **A1** (`1a5977b`) — setWage lever typed + clamped (no-op): `BusinessAction.setWage`, `DecisionLimits.min/maxWagePerTick` (0..1 safety band). Nothing emits it yet → byte-identical, 271 tests.
- **A2** (`83aaa31`) — apply setWage + re-rate sitting staff (no-op): immutable `Business.baseWagePerTick` (seeded = posted wage); `applySetWage` clamps to `[base, base*MAX_WAGE_MULT]`, sets the posted wage, re-rates sitting staff *up* (never cut); resident `jobBaseWage` now reads the stable base so raise caps can't compound. Moves no cash. Still inert → 273.
- **F2** — lever-ablation harness (the "prove the control surface is real" deliverable; F1 capital-scoring already shipped in the E1+F1 bundle). New `AblatedProvider` (runs the rules mind but deletes one lever from its action) + `ablationStudy()` + `formatAblation()`, wired into `bench/cli.ts`. **Findings (honest, and exactly what the harness is for):** `setPrice` and `invest` are genuine controls — they move net worth by real margins ($1k+ in a churn scenario) — and crucially their *sign is context-dependent*: investing into slack demand **costs** net worth (a good CEO invests only when capacity-bound), the mark of a real control surface rather than free-money buttons. `hire`/`setWage` read **~0** because the A3 hiring cap keeps staffing **static** (nothing can poach a fully-crewed firm), so these are *recovery levers* — dormant insurance that bites only under the disruption that **business entry/exit (D)** introduces (a dead firm frees workers; a new firm bids for them). So **F3** ("invest+hire+setWage out-build pricing") is empirically premature — the labour levers prove out with D, not before — and is folded into this finding rather than forced. **279 green** (+1 ablation test that asserts the active levers bite and *catches the dormant ones*).
- **C** — real ownership (owner-dividend split). A share λ of each firm's daily distributable profit now goes to its **owner** before the rest recirculates evenly; the `ownerId` field is finally load-bearing. **Owners end ~2× wealthier** than non-owners over an agentic year (cash + luxuries) — owning (and, in D, founding) a firm now pays. Two findings forced the design: (1) the split must stay *modest* — at λ=0.35 the wealth concentration pushed demand out of band and broke the 13c loop + the competition truce (rich owners hit the demand cap and hoard, *shrinking* aggregate demand and hurting the higher-capacity firm most), so live λ is **0.1**; (2) on the **CEO bench** the dividend made the sacred `rules>off` gap *chaotic* in λ (firm net worth can't cleanly score skill once profit leaks to the owner's pocket) — so the bench **freezes the dividend to 0** (`BENCH_OWNER_DIVIDEND_SHARE`, plumbed like `BENCH_WEALTH_ELASTICITY`), keeping its score a clean skill signal while the live game keeps the dividend. The split is conservation-safe (every cent still moves via `World.transfer`). **278 green** (+2 tests: owners wealthier; the dividend *itself* widens the gap vs frozen-0, isolating it from the incidental owner/job correlation).
- **E3** — re-fire the invest loop (14c). **Finding: the loop already fires now that the produce exploit is gone (E1 unmasked utilization) and head-count drives output (E2)** — a full-agentic year deepens total capital to ≈3× baseline (the 13c test now asserts material `>1.5×` deepening, not a token blip). Measured `TARGET_CAPITAL_SCALING` on vs off over a 2-year run: turning it **on was worse** (a firm went insolvent *earlier*, capital ended lower), so it **stays off** — the 14a/14c "scale the target with capital" theory is empirically rejected now that producers actually survive. The engine is *real but not perpetual*: capital peaks ~day 180 then decays slowly over years as depreciation outpaces a maturing city's reinvestment, and a producer can fail in year 2 — **multi-year firm churn is handled by business entry/exit (slice D), not by inflating buffers.** North-Star #3 (close the investment loop) delivered: the lever fires and capital deepens materially. **276 green.**
- **A3 + A4 + E2** (the labour market, shipped together — they're one inseparable change). **The supply chain now survives full agentic play.** Pieces: A3 caps each firm's `hiring` signal at a desired headcount so agentic residents can't all stampede the top-wage storefront (P10-3); A4 lets the rules brain bid wages up when short-handed (and ease back when fully-staffed-but-cash-thin, so wages don't ratchet to the cap); E2 sets `LABOR_FULL_STAFF=2` so head-count *drives output* (2 = full, 1 = half), making a poached worker a real loss and `hire` a real recovery lever. Supporting: a `desiredHeadcount(kind)` helper (0 for the non-producing landlord); cityGen reseeds so workers staff only producing firms (the **landlord runs crewless**), exactly filling the 6 producers at 2 each — so brain-off output is **unchanged** (2 workers → laborFactor 1 → today's maxPerDay); the rules-brain hire lever is capped at `understaffed` so a profitable storefront can't vacuum the jobless pool. **Probe (full-agentic year, all 6 producers + 12 residents agentic): zero deaths, every producer crewed at 2, wages settle near base (no ratchet), invest fires 24×, goods capital deepens to ~1700.** The decisive fix was *coupling* DESIRED_HEADCOUNT to LABOR_FULL_STAFF: decoupled (desired 2, full-staff 1) made the 2nd worker pure overhead → producers ran unprofitable and died; coupled, both workers are output-justified and the cost-plus floor covers them. **276 tests green** (+5 locking tests: headcount→output, the 3 wage heuristics, the hiring cap; the 13c year now also asserts producers stay crewed). Re-baseline: 2 resident-switch tests open a vacancy first (a full firm advertises none).
- **E1 + F1** (bundled) — remove the produce exploit lever **and** count capital in net worth, together. Why bundled: the probe proved they're complementary. Removing `produce` (a free-COGS exploit that also *masked utilization*, the 14c blocker) dropped the rules CEO **below** the no-op on cash+inventory (−1196) — because the rules CEO already invests (5×/42 turns, capital → 1839, now that utilization is unmasked) but the bench wasn't counting capital. **F1** scores capital at depreciated above-baseline book → rules **+543** over off, restoring the sacred `rules > off` ordering via the *legitimate* invest lever instead of the exploit. Two honest re-baselines: the bench net-worth identity now includes capital; the competition price-war test opens at a **viable** undercut (14 not 12) — a below-cost war is correctly no longer survivable without the free-restock exploit (distribution skims a marginal firm's good days but not its bad ones, so it bleeds out), while viable undercuts still converge to an exact shared price with both surviving. `produce` removed from the type, clamp, both providers, apply, and the UI. **271 tests green.** _(F1 pulled forward from F-partial; F2 ablation + F3 producer-CEO still pending.)_
- **D** — business entry/exit (creative destruction). **D1** (`aaf2293`): on bankruptcy the husk's residual cash goes to its owner (recouped equity) and its stock is written off, behind `RECYCLE_BANKRUPT_ASSETS` (on) — dead money returns to circulation instead of freezing. **D2+D3** (`5dbb10c`): a new `BusinessEntrySystem` (after Lifecycle, before Macro) — once a `BusinessKind` goes fully extinct, the lowest-index resident with savings founds a fresh firm to serve the standing demand: funded from their own pocket (a transfer, no mint), staffed from the jobless pool (the dead firm's freed crew), **owned by the founder** (so the dividend rewards the risk). The newborn reuses the dead firm's location/template, and **`MarketSystem` now finds producers by kind** (a dynamic lookup, not the fixed `PRODUCER_OF` id — a no-op for the seeded city) so a respawned producer slots straight into the supply chain. Deterministic (fixed kind order, lowest-index founder, a serialized spawn counter), conservation-safe, cooldown-spaced; behind `BUSINESS_ENTRY` (on) but inert until a kind dies, so the seeded city is byte-identical. Tuning reality: residents run cash-light (~$500–950), so the founder bar is low (`ENTREPRENEUR_MIN_SAVINGS=600`, `NEW_FIRM_CAPITAL=500`) — a firm born into an *empty* niche is the sole supplier and earns its way up (probe: a killed diner's replacement grew $500→$3000). The lifecycle tests gained a `businessEntry:false` toggle to isolate a death from the entry that would otherwise refill it. **D4**: a self-healing soak — forced exits on two chains are both refilled over a churning run, money conserved, nobody underwater. **286 green** (+6 entry tests + 1 liquidation test). **This is the disruption that finally makes the labour levers bite** (F2's dormant `hire`/`setWage`): a newborn firm bids for the workers a dead one freed.
- **F4** — the whole-arc soak. Three years of the *full* living economy — all six firms agentic, all twelve residents agentic, the owner dividend, business birth/death, and disasters, all at once — on seeds 1 & 7 holds **every invariant**: money conserved to the cent, needs bounded, no holder underwater, no NaN, a core of ≥4 kinds always served (entry refills what churn kills), and positive GDP on the final day. Plus a multi-year determinism check (identical world from identical seed). **288 green.**

## ✅ Phase 15 COMPLETE

The living firm economy works end to end: producers survive agentic play and price above cost (**B**); firms compete for labour and head-count drives output (**A1–A4, E2**); the produce exploit is gone and the invest engine fires, deepening capital ~3× (**E1, E3**); capital is scored and every lever's worth is measured (**F1, F2**); owners earn their firm's profit (**C**); and firms are **born and die**, the city self-healing its empty niches (**D**) — which is the churn that finally makes the labour levers bite. All money-conserved to the cent, deterministic from seed+snapshot, needs bounded, stable over a 3-year whole-arc soak. North-Star **#2** (firm health / business entry) and **#3** (productivity engine + profit→owners) delivered.

Honest limits carried forward (candidates for a future phase): the productivity engine is *real but not perpetual* (capital decays over years; D's churn, not a Solow loop, sustains the long run); the labour levers are *recovery insurance* (A3's cap does the steady-state heavy lifting); the owner dividend is kept modest (λ=0.1) because the economy is demand-sensitive to wealth concentration.
