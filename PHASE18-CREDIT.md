...
| **18f** | Default settlement: husk `→ bank` before `→ owner` in `LifecycleSystem`; unrecovered debt = real bank loss. |
| **18g** | Observation slice: surface `debtPrincipal/debtInterest/borrowed/debtServicePaid/creditRate` + **net financing out of `dayProfit`/`dayRent`** (read-only). |
